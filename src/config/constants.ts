 ...
 * 📌 系统概述：
 * - 9维性格系统用于量化修士的性格特质
 * - 通过问心试炼的答案进行评分，影响灵根分配和问道包匹配
 * - 评分范围：-3 到 +8（负面特征可为0，正面特征可为正数）
 *
 * 🔧 评分机制：
 * - 选择题：固定选项加分（客观评分）
 * - 开放题：AI智能解析（主观评分）
 * - 最终得分 = 选择题权重 × 选择题得分 + 开放题权重 × 开放题得分
 *
 * 📝 性格维度说明：
 * - determination: 决断力 - 面对选择时的果敢程度
 * - courage: 勇气 - 面对危险时的勇敢程度
 * - stability: 稳定性 - 情绪和行为的稳定程度
 * - focus: 专注力 - 注意力集中和自律程度
 * - honesty: 诚实 - 真诚作答和自我反省程度
 * - kindness: 善良 - 同情心和利他主义程度
 * - greed: 贪念 - 过度追求利益的程度（负面特征）
 * - impatience: 急躁 - 缺乏耐心和冲动的程度（负面特征）
 * - manipulation: 操控 - 试图操纵系统或作弊的程度（负面特征）
 *
 * ⚠️ 注意事项：
 * - 负面特征（greed, impatience, manipulation）最小值为0，不能为负
 * - 正面特征可为负数，表示该特质缺失
 * - 显著阈值用于判断性格特征是否突出
 */
export const PersonalityConfig = {
   ...
   */
  DIMENSIONS_V1: [
    'determination',  // 决断力 - 快速做决定，不犹豫
    'courage',        // 勇气 - 敢于面对危险和挑战
    'stability',      // 稳定性 - 冷静、不冲动、情绪稳定
    'focus',          // 专注力 - 注意力集中、深思熟虑
    'honesty',        // 诚实 - 真诚作答、不作弊、不自欺
    'kindness',       // 善良 - 同情心、利他主义、仁慈
    'greed',          // 贪念 - 过度追求利益（负面特征）
    'impatience',     // 急躁 - 缺乏耐心、冲动（负面特征）
    'manipulation',   // 操控 - 试图操纵系统、作弊（负面特征）
  ] as const,
   ...
   *
   * 📊 范围说明：
   * - MIN_SCORE: 0 - 所有维度的最低分
   * - MAX_SCORE: 10 - 理论最高分（实际AI评分上限为8）
   *
   * 💡 实际使用：
   * - AI评分时会自动约束在合理范围内
   * - 负面特征最小值为0（不能为负）
   */
  MIN_SCORE: 0,//作为展示参考，更改无效果
  MAX_SCORE: 10,// 作为展示参考，更改无效果

  /**
   * 性格特征显著阈值
   *
   * 📈 阈值说明：
   * - POSITIVE_TRAIT_THRESHOLD: 正面特征 >= 5分才算显著
   * - NEGATIVE_TRAIT_THRESHOLD: 负面特征 >= 3分就算显著
   *
   * 🎯 使用场景：
   * - 灵根分配算法：显著特征获得额外概率加成
   * - 问道包匹配：显著特征影响匹配度计算
   * - 性格分析：生成性格描述时参考阈值
   */
  POSITIVE_TRAIT_THRESHOLD: 5,  // 正面特征>=5才显著//作为展示参考，更改无效果
  NEGATIVE_TRAIT_THRESHOLD: 3,  // 负面特征>=3就显著// 作为展示参考，更改无效果
   ...
   *
   * ⚙️ 配置说明：
   * - V2_AI_TIMEOUT_SECONDS: AI分析超时时间（秒）
   * - V2_FALLBACK_TO_V1: 失败时是否降级到v1.0系统
   *
   * 🚧 当前状态：实验性功能，未投入使用
   */
...
export const PathPackageConfig = {
  ...
  GOOD_MATCH_THRESHOLD: 0.5,      // >=50% 良好匹配
  // <50% 普通匹配
